...
/***************************************************************
 *  Copyright notice
 *  (c) 2013 Martin Gjoshevski <gosevski@gmail.com>
 *  All rights reserved
...
class CategoryService {
	...
	private static function getChildrenCategoriesRecursive($idList, $counter, $additionalWhere) {
		...
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'uid',
			'sys_category',
			'sys_category.parent IN (' . $GLOBALS['TYPO3_DB']->cleanIntList($idList) . ') AND deleted=0 ' . $additionalWhere
		);
		while(($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))) {
			$counter++;
			if ($counter > 10000) {
				$GLOBALS['TT']->setTSlogMessage('EXT:product_catalog: one or more recursive categories where found');
